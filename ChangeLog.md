# Changelog

## Version [2.0.1](https://github.com/CedX/Lcov.ps1/compare/v2.0.0...v2.0.1)
- Optimized the packaging.

## Version [2.0.0](https://github.com/CedX/Lcov.ps1/compare/v1.1.0...v2.0.0)
- Replaced the [PowerShell](https://learn.microsoft.com/en-us/powershell) classes by [C#](https://learn.microsoft.com/en-us/dotnet/csharp) classes.
- Restored the module's `DefaultCommandPrefix`.

## Version [1.1.0](https://github.com/CedX/Lcov.ps1/compare/v1.0.1...v1.1.0)
- Removed the module's `DefaultCommandPrefix` in favor of a hard-coded command prefix.

## Version [1.0.1](https://github.com/CedX/Lcov.ps1/compare/v1.0.0...v1.0.1)
- Fixed a packaging issue.

## Version 1.0.0
- Initial release.
